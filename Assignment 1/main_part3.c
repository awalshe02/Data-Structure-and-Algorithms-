...
int hash3(char* s){ //get hash number
    int hash = 0;
    while(*s){
...
int insert(person *p) {
    ...
    int hindex = hash_function(p->name); //get hash index
        ...
        for (int i = 0; i < ARRAY_SIZE; i++) {
           int  next = (hindex + (i * hash3(p->name))) % ARRAY_SIZE; //double hashing
...
person *search(char *name){
    ...
    for (int i = 0; i < ARRAY_SIZE; i++) {
        int next = (hindex + (i * hash3(name))) % ARRAY_SIZE; //double hashing
...
void addORIncrement(char* name){
    ...
    if(p != NULL){ //person is found and num is returned
        p->value++; 
    }   
    else{
        create_new_person(name); //if new name, emory is made
    }
}
