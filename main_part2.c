...
typedef struct person { //hash function
...
unsigned long hash2(char* s) { //get hash number
...
int insert(person *p) {
    ...
    unsigned long hindex = hash2(p->name); //get hash index

        for (int i = 0; i < ARRAY_SIZE; i++) {
           unsigned long next = (i + hindex) % ARRAY_SIZE; //linear probing
...
person *search(char *name){
    ...
    unsigned long hindex = hash2(name); //get hash index
